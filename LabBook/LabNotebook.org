...
int main(void){
  ...
  for( int iFile = 0; iFile < numFiles; iFile++ ){
    cc = BetaDist["reco"][charges[iFile]][masses[iFile]]->canvas;
    ch = BetaDist["reco"][charges[iFile]][masses[iFile]]->distribution;
    cc = new TCanvas("beta","beta",500,500);
    //Giving the same upper and lower bounds auto calculates axis range in x
    ch = new TH1F("beta","beta",2,2);   
  }
  ...
  for( int iFile = 0; iFile < numFiles; iFile++ ){
    ...
    for(int iEvt = 0; iEvt < tree->GetEntriesFast(); iEvt++){
      ...
      BetaDist["reco"][charges[iFile]][masses[iFile]]->distribution->Fill(beta);
