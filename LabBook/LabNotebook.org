...
* TODO Things to do [12/13]                                          :export:
  [-] Write a script to automate the creation of the generator txt files. They can be located in the Configuration/Generator/data/      folder and resemble the following: "particles_HIP%d_%s_%d_GeV.txt" %(CHARGE(e/3),FLAVOR(stau in our case),MASS(GeV))
    [-] Create arrays of all desired charges (e/3) and masses
    [-] Loop through each combination of these. For each combination, insert the necessary charge and mass into a cat> blah.txt<<EOF as usual. Use one of the existings as your starting point, of course.
  [-] Write a script to automate the creation of the generator tbl files. They can be found in the Configuration/Generator/data folder and resemble the following: "hscppythiapdtHIP%d%s%d.tbl" %(CHARGE,FLAVOR,MASS)
    [-] Same as above for the creation
  [-] Write a script to automate the creation of the generator cff files. They can be located in the Configuration/GenProduction/python/ThirteenTeV folder and resemble the following: "HSCPmchamp%d_M_%d_TuneZ2star_13TeV_pythia6_cff.py" %(CHARGE,MASS)
  [-] After completing all of the above, we need to compile the python files by running the SCRAM command from the analysis page.
